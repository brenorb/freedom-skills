...
metadata:
  ...
  prerequisites:
    commands:
      - python3
  permissions:
    - read/write local state under PDD_DATA_DIR or HERMES_HOME/unbroker
    - network access to broker sites, public registries, and configured SMTP/IMAP endpoints
    - browser automation for consented opt-out and verification flows
    - launch a dedicated local browser profile for CDP-based webmail workflows
  hermes:
    tags:
      - privacy
      - data-broker
