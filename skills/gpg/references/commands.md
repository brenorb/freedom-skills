...
gpg --batch --yes --symmetric --armor --output secret.txt.sym.asc secret.txt
```
...
```bash
gpg --batch --yes --armor --output message.txt.asc --clear-sign message.txt
gpg --batch --yes --armor --output release.tar.gz.sig --detach-sign release.tar.gz
gpg --batch --yes --armor --output note.asc --sign note.txt
