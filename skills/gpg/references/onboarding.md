...
gpg --symmetric --cipher-algo AES256 --output gpg-test.txt.gpg gpg-test.txt
gpg --decrypt --output gpg-test.out.txt gpg-test.txt.gpg
cat gpg-test.out.txt
```

For recipient encryption after a keypair exists locally:
...
```bash
gpg --armor --output gpg-test.txt.sig --detach-sign gpg-test.txt
