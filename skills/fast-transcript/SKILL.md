...
```bash
bash skills/fast-transcript/scripts/fscript_info.sh "<audio-or-url>"
bash skills/fast-transcript/scripts/fscript_info.sh "<audio-or-url>" "/tmp/transcript.txt"
```
...
```bash
fscript "<audio-or-url>" - --text=timestamps -D
```
...
```bash
fscript "<audio-or-url>" - --speakers=timestamps -d
```
